...
from voicemath.helpers.exceptions import GrammarError
from voicemath.hme_parser import check_grammar_lex as check_grammar_lex
from voicemath.hme_parser import correct_grammar as correct_grammar
from voicemath.hme_parser.grammar import yacc as yacc
...
class CheckSintax():
            ...
            if error['value'] is not None:
                ...
                for symbol in latex:
                    ...
                    if symbol['label'] == latex_error_token and count == latex_error_pos:
                        yacc_errors.append({'pos': latex_error_pos, 'pos_list': count_list, 'label': symbol['label'],
    ...
    def check_correct_grammar(self):
        ...
        if not self.yacc_error_list and self.__first_error and self.attempts < 3 and self.latex_string:
            ...
            raise GrammarError({'latex': self.latex, 'latex_list': self.latex_list, 'latex_string': self.latex_string,
                                'error': self.yacc_error_list,  # Current error
                                'errors_history': self.yacc_errors_history, 'pure_errors': self.pure_yacc_errors})
        ...
        return {'latex': self.latex, 'latex_list': self.latex_list, 'latex_string': self.latex_string,
                'yacc_errors_history': self.yacc_errors_history, 'lex_errors_history': self.lex_errors_history,
                'yacc_pure_errors': self.pure_yacc_errors, 'lex_pure_errors': self.pure_lex_errors}
